                ...
                formatter: function (value, row, index) {//这里的三个参数：value表示当前行当前列的值；row表示当前行的数据；index表示当前行的索引（从0开始）。
					...
					if(row.delay && row.appoint){
						html = '<div style=\'width:100%;\'>' +
							...
							'<button type="button" onclick="delayModel(' + row.projectId + ')" class="btn btn-danger"  style="font-weight:150;font-size:12px;padding:3px 8px;margin:5px;"><span class="glyphicon glyphicon-remove" aria- hidden="true" ></span >申请延期</button >'+
							'<button type="button" onclick="examine (' + row.projectId +','+row.taskName+ ')" class="btn btn-primary"  style="font-weight:150;font-size:12px;padding:3px 8px;margin:5px;"><span class="glyphicon glyphicon-pencil" aria- hidden="true" ></span >延期审核</button >'+
							'</div>';
					} else if(row.delay){
						html = '<div style=\'width:100%;\'>' +
		                '<button type="button" onclick="updateUserTask(' + row.id + ')" class="btn btn-primary"  style="font-weight:150;font-size:12px;padding:3px 3px;margin:5px;"><span class="glyphicon glyphicon-pencil" aria- hidden="true" ></span >确认完成</button >&nbsp;&nbsp;'+
